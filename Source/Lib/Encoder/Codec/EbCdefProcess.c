    ...
    for (fbr = y_b64_start_idx; fbr < y_b64_end_idx; ++fbr) {
        for (fbc = x_b64_start_idx; fbc < x_b64_end_idx; ++fbc) {
            ...
            if (((fbc & 1) &&
                ...
                continue;
            if (sb_type == BLOCK_128X128 || sb_type == BLOCK_128X64 ||
                sb_type == BLOCK_64X128)
                bs = sb_type;
            ...
            for (pli = 0; pli < num_planes; pli++) {
                ...
                */
                ...
                for (gi = 0; gi < first_pass_fs_num; gi++) {
                    ...
                    if (!pli || (cdef_ctrls->default_first_pass_fs_uv[gi] != -1)) {
                        ...
                        curr_mse = svt_compute_cdef_dist_8bit(
                            ref_coeff[pli] +
                            (lr << mi_high_l2[pli]) * stride_ref[pli] +
                            (lc << mi_wide_l2[pli]),
                            stride_ref[pli],
                            tmp_dst,
                            dlist,
                        ...
                        if (pli < 2)
                            pcs_ptr->mse_seg[pli][fb_idx][gi] = curr_mse * subsampling_factor;// Phoenix - *2 b/c using every other line
                        else
                        ...
                        if (pli < 2)
                            ...
                            pcs_ptr->mse_seg[1][fb_idx][gi] += curr_mse;
#endif
#if MS_CDEF_OPT2
                    }
                    else
                 ...
                 */
                for (gi = first_pass_fs_num; gi < first_pass_fs_num + default_second_pass_fs_num; gi++) {
#if MS_CDEF_OPT2
                    if (!pli || (cdef_ctrls->default_second_pass_fs_uv[gi - first_pass_fs_num] != -1)) {
#endif
                        pri_strength = cdef_ctrls->default_second_pass_fs[gi - first_pass_fs_num] / CDEF_SEC_STRENGTHS;
                        sec_strength = cdef_ctrls->default_second_pass_fs[gi - first_pass_fs_num] % CDEF_SEC_STRENGTHS;
        ...
        for (fbc = x_b64_start_idx; fbc < x_b64_end_idx; ++fbc) {
            ...
            for (pli = 0; pli < num_planes; pli++) {
                ...
                for (gi = start_gi; gi < end_gi; gi++) {
                    ...
                    get_cdef_filter_strengths(pick_method, &threshold, &sec_strength, gi);
                    svt_cdef_filter_fb(tmp_dst,
                                       NULL,
                                       CDEF_BSTRIDE,
                                       in,
