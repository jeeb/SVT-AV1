...
EbErrorType av1_inter_prediction_16bit_pipeline(
            ...
            for (int32_t y = 0; y < b8_h; y += b4_h) {
                ...
                for (int32_t x = 0; x < b8_w; x += b4_w) {
                    ...
                    if (this_mbmi->block_mi.ref_frame[0] == LAST_FRAME ||
                        ...
                        ref_pic =((EbReferenceObject *)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_0][ref_idx]->object_ptr)->reference_picture;

                    else
                        ref_pic =((EbReferenceObject *)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_1][ref_idx]->object_ptr)->reference_picture;
    ...
    if (mv_unit->pred_direction == UNI_PRED_LIST_0 || mv_unit->pred_direction == BI_PRED) {
        ...
        if (ref_pic_list0->buffer_bit_inc_y) {
            ...
            src_ptr_10b = src_ptr_10b + (mv_q4.row >> SUBPEL_BITS) * ref_pic_list0->stride_y + (mv_q4.col >> SUBPEL_BITS);
            ...
            if (plane == 0) {
                ...
                if (pu_origin_y != 0 && pu_origin_x != 0)
                    ...
                    ((uint16_t *)luma_recon_neighbor_array
                        ->top_left_array)[luma_recon_neighbor_array->max_pic_h + pu_origin_x - pu_origin_y];
#else
                if (pu_origin_y != 0 && pu_origin_x != 0)
                    topNeighArray[0] = leftNeighArray[0] =
                    ((uint16_t *)luma_recon_neighbor_array
                ...
                if (blk_originy_uv != 0 && blk_originx_uv != 0)
                    ...
                    ((uint16_t *)cb_recon_neighbor_array
                        ->top_left_array)[cb_recon_neighbor_array->max_pic_h + blk_originx_uv -
                    blk_originy_uv / 2];
#else
                if (blk_originy_uv != 0 && blk_originx_uv != 0)
                    topNeighArray[0] = leftNeighArray[0] =
                ...
                if (blk_originy_uv != 0 && blk_originx_uv != 0)
                    ...
                    ((uint16_t *)cr_recon_neighbor_array
                        ->top_left_array)[cr_recon_neighbor_array->max_pic_h + blk_originx_uv -
                    blk_originy_uv / 2];
#else
                if (blk_originy_uv != 0 && blk_originx_uv != 0)
                    topNeighArray[0] = leftNeighArray[0] =
