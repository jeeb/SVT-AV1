...
#if FTR_TPL_SYNTH
    uint8_t tpl_synth_size_offset = pcs_ptr->tpl_ctrls.synth_blk_size  == 8  ? 0 : pcs_ptr->tpl_ctrls.synth_blk_size  == 16 ? 1 : 2;
    const uint32_t blks                 = scs_ptr->seq_header.sb_size == BLOCK_128X128
                        ? (128 >> (3 + tpl_synth_size_offset))
                        : (64 >> (3 + tpl_synth_size_offset));
#else
    const uint32_t blks                 = scs_ptr->seq_header.sb_size == BLOCK_128X128
        ...
        for (uint32_t sb_x = 0; sb_x < picture_sb_width; ++sb_x) {
            ...
            for (uint32_t blky_offset = 0; blky_offset < blks; blky_offset++) {
                for (uint32_t blkx_offset = 0; blkx_offset < blks; blkx_offset++) {
#if FTR_TPL_SYNTH
                    uint32_t blkx = (((sb_x * sb_sz) >> (3 + tpl_synth_size_offset)) +
                        blkx_offset);
                    uint32_t blky = (((sb_y * sb_sz) >> (3 + tpl_synth_size_offset)) +
                        blky_offset);
                    uint32_t blkx_temp = blkx;
                    uint32_t blky_temp = blky;
                   ...
                   if ((blkx_temp ) >= picture_width_in_mb ||
                       (blky_temp ) >= picture_height_in_mb)
#else
...
#if OPT_TPL_DATA
                    TplStats *tpl_stats_ptr =
                        pcs_ptr->pa_me_data->tpl_stats[blky * (mi_cols_sr >> shift) + blkx];
                    int64_t mc_dep_delta = RDCOST(pcs_ptr->pa_me_data->base_rdmult,
                        tpl_stats_ptr->mc_dep_rate,
                        tpl_stats_ptr->mc_dep_dist);
#else
